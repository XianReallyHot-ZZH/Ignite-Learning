...
description: 从零手搓 Ignite —— 为某 session 写真实代码 + 讲义(stage ③)。当该 session 执行规格已存在、用户要"写代码/实现 S??/建 sNN 工程"时使用。按执行规格 §4 实现、§2 对外契约、§5 具名测试;在 ignite-gogogo/sNN-*/ 建独立多模块 Maven 工程,mvn 跑绿并核验具名测试;代码完成后写讲义(docs-learn/,描述实际产物,check-handouts 把关)。调用 /ignite-session-code <NN>。
---

# ignite-session-code &lt;NN&gt;

为 **session NN** 实现代码工程、跑绿、并写讲义(描述实际产物)。

## 前置
执行规格 `specs/sessions/SNN-*.md` 存在。没有则先 `/ignite-session-doc <NN>`。

## 流水线位置
stage ③:**分析 → 执行规格 → [代码 → 注释 → 讲义]**。讲义在代码完成后写(描述实际建出的类/链路/差距,避免推测)。
...
- **讲义描述实际产物**:架构图 / 链路图 / 对照都据**真实代码**写,不推测;讲义必写(`scripts/check-handouts.sh` 把关)。

## 完成 =
工程构建 + 测试绿(mvn)+ §5 具名测试全部存在且绿 + **注释齐全(补完复跑绿)** + **讲义(描述实际产物,check-handouts 过)** + PROGRESS 更新。
